package com.example.chrisnguyen.customkeyboard.constants;

/**
 * Created by chrisnguyen on 12/11/16.
 */
public class Quotes {

    public static final String[] SIGNATURES = {
            "\n- That's what Trump said",
            "\n- You've been Trumped!",
            "\n- The D"
    };

    public static final String[] CATEGORIES = {
            "Recent",
            "Arrogant",
            "Disapproving",
            "Disgusting",
            "Inspirational",
            "Sarcastic",
            "Sexist",
            "Stupid",
            "Supremacist",
    };

    public static final String[] arrogant = {
            "All of the women on The Apprentice flirted with me – consciously or unconsciously. That's to be expected.",
            "I think the only difference between me and the other candidates is that I'm more honest and my women are more beautiful.",
            "I'm just thinking to myself right now, we should just cancel the election and just give it to Trump, right.",
            "Look at those hands, are they small hands? And, [Republican rival Marco Rubio] referred to my hands: 'If they're small, something else must be small.' I guarantee you there's no problem. I guarantee.",
            "My fingers are long and beautiful, as, it has been well documented, are various other parts of my body.",
            "My IQ is one of the highest — and you all know it! Please don't feel so stupid or insecure; it's not your fault.",
            "My whole life is about winning. I don't lose often. I almost never lose.",
            "People love me. And you know what, I have been very successful. Everybody loves me.",
            "The beauty of me is that I'm very rich!",
            "You know, it really doesn't matter what the media write as long as you've got a young, and beautiful piece of ass.",
    };

    public static final String[] disapproving = {
            "I think Bush is probably the worst President in the history of the United States.",
            "(John McCain is) not a war hero…. He's a war hero because he was captured. I like people that weren't captured.",
            "One of they key problems today is that politics is such a disgrace. Good people don't go into government.",
            "Our great African-American President hasn't exactly had a positive impact on the thugs who are so happily and openly destroying Baltimore.",
            "The other candidates — they went in, they didn't know the air conditioning didn't work. They sweated like dogs… How are they gonna beat ISIS? I don't think it's gonna happen.",
    };

    public static final String[] disgusting = {
            "During a debate in response to a question about him calling women, \"fat pigs, dogs, slobs, and disgusting animals\": \"Only Rosie O'Donnell.\"",
            "I have never seen a thin person drinking Diet Coke.",
            "If I were running 'The View', I'd fire Rosie O'Donnell. I mean, I'd look at her right in that fat, ugly face of hers, I'd say 'Rosie, you're fired.'",
            "Look at that face! Would anyone vote for that? Can you imagine that, the face of our next president?! I mean, (Carly Fiorina's) a woman, and I'm not supposed to say bad things, but really, folks, come on. Are we serious?",
            "Sorry, there is no STAR on the stage tonight!",
            "You're disgusting.",
    };

    public static final String[] inspirational = {
            "I've said if Ivanka weren't my daughter, perhaps I'd be dating her.",
            "The point is, you can never be too greedy.",
            "To his supporters about protesters at a rally: \"If you see somebody with a tomato, knock the crap out of them.\"",
            "When somebody challenges you, fight back. Be brutal, be tough.",
            "You have to think anyway, so why not think big?"
    };

    public static final String[] sarcastic = {
            "I had great relationship with the Hispanic - we had a lot of Hispanics in the school actually from different countries, Venezuela, from Brazil, and they all played soccer, and I was on the soccer team, and I developed great relationships with them.",
            "I was down there, and I watched our police and our firemen, down on 7-Eleven, down at the World Trade Center, right after it came down.",
            "Number one, I have great respect for women. I was the one that really broke the glass ceiling on behalf of women, more than anybody in the construction industry.",
            "Thanks sweetie. That's nice.",
    };

    public static final String[] sexist = {
            "I have so many fabulous friends who happen to be gay, but I am a traditionalist.",
            "The only card [Hillary Clinton] has is the woman's card. She's got nothing else to offer and frankly, if Hillary Clinton were a man, I don't think she'd get 5 percent of the vote. The only thing she's got going is the woman's card, and the beautiful thing is, women don't like her.",
            "She's back there, little Katy... a third-rate reporter.",
    };

    public static final String[] stupid = {
            "How stupid are the people of Iowa?",
            "I'm afraid the election is going to be rigged, I have to be honest.",
            "It's freezing and snowing in New York – we need global warming!",
            "My Twitter has become so powerful that I can actually make my enemies tell the truth.",
            "We're losing a lot of people because of the Internet. We have to see Bill Gates and a lot of different people who really understand what's happening and maybe, in some ways, closing that Internet up in some ways.",
    };

    public static final String[] supremacist = {
            "I will build a great wall – and nobody builds walls better than me, believe me – and I'll build them very inexpensively. I will build a great, great wall on our southern border, and I will make Mexico pay for that wall. Mark my words.",
            "We will make America strong again. We will make America proud again. We will make America safe again. And we will make America great again.",
            "When Mexico sends its people, they're not sending the best. They're not sending you, they're sending people that have lots of problems and they're bringing those problems with us. They're bringing drugs. They're bring crime. They're rapists… And some, I assume, are good people."
    };

}
